...
import type { DbManager } from "../db/db-manager.ts";
const logger = getLogger(["build", "module-manager"]);
...
export class ModuleManager {
    ...
    if (!mod.mod) {
      try {
        ...
        if (mod_exports && mod_exports.default) {
          ...
          logger.error(`module export default not found` + mod_exports.default+' which url is: '+mod.url);
        }
      } catch (e: Error) {
        logger.error(e);
        result.ok = false;
  ...
  async handle_request(c: Context<{Variables:{db_manager:DbManager}}>) {
    ...
    if (handle_mod) {
      if (handle_mod.mod) {
        ...
        return await handle_mod.mod.request(
          url.href,
          c.req.raw,
          { module_path, db_manager: c.get("db_manager"),module_manager:this },
        );
      }
    } else {
      //
